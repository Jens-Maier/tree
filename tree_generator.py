            ...
            for clusterIndex in range(0, branchClusters):
                ...
                if len(startNodesNextIndexStartTvalEndTval) > 0:
                    ...
                    for branchIndex in range(0, nrBranches):
                        ...
                        startPointTangent = utils_.utils.sampleSplineTangentT(data.startNode.point, 
                                                                data.startNode.next[startNodeNextIndex].point, 
                                                                data.tangent, 
                                                                data.startNode.next[startNodeNextIndex].tangent[0], 
                                                                data.t)
                                                                
                        branchStartTvalGlobal = utils_.utils.lerp(data.startNode.tValGlobal, data.startNode.next[startNodeNextIndex].tValGlobal, data.t)
                        
                        globalVerticalAngle = utils_.utils.lerp(branchClusterSettingsList[clusterIndex].verticalAngleCrownStart, branchClusterSettingsList[clusterIndex].verticalAngleCrownEnd, data.startNode.tValGlobal)
                        
                        branchVerticalAngle = utils_.utils.lerp(branchClusterSettingsList[clusterIndex].verticalAngleBranchStart, branchClusterSettingsList[clusterIndex].verticalAngleBranchEnd, utils_.utils.lerp(data.startNode.tValBranch, data.startNode.next[data.startNodeNextIndex].tValBranch, data.t))
                        
                        verticalAngle = globalVerticalAngle + branchVerticalAngle
                        
                        globalRotateAngle = utils_.utils.lerp(branchClusterSettingsList[clusterIndex].rotateAngleCrownStart, branchClusterSettingsList[clusterIndex].rotateAngleCrownEnd, branchStartTvalGlobal)
                        
                        branchRotateAngle = utils_.utils.lerp(branchClusterSettingsList[clusterIndex].rotateAngleBranchStart, branchClusterSettingsList[clusterIndex].rotateAngleBranchEnd, utils_.utils.lerp(data.startNode.tValBranch, data.startNode.next[data.startNodeNextIndex].tValBranch, data.t)) # tValBranch == 0 !!!
                        ...
                        startTvalGlobal = utils_.utils.lerp(data.startNode.tValGlobal, data.startNode.next[startNodeNextIndex].tValGlobal, data.t)
                        startTvalBranch = utils_.utils.lerp(data.startNode.tValBranch, data.startNode.next[startNodeNextIndex].tValBranch, data.t)
                        ...
                        if branchClusterSettingsList[clusterIndex].branchType.value == "WHORLED":
                            whorlCount = int(round(utils_.utils.lerp(branchClusterSettingsList[clusterIndex].branchWhorlCountStart, branchClusterSettingsList[clusterIndex].branchWhorlCountEnd, startTvalGlobal)))
        ...
        for s in range(0, len(segments)):
            ...
            if segmentLength > 0:
                ...
                for section in range(startSection, sections + 1):
                    pos = utils_.utils.sampleSplineC(segments[s].start, controlPt1, controlPt2, segments[s].end, section / sections)
                    tangent = utils_.utils.sampleSplineTangentC(segments[s].start, controlPt1, controlPt2, segments[s].end, section / sections).normalized()
                    
                    if section == 0:
                        tangent = segments[s].firstTangent
                    
                    dirA = utils_.utils.lerp(segments[s].startCotangent, segments[s].endCotangent, section / sections)
                    ...
                    if segments[s].clusterIndex == -1:
                        taper = utils_.utils.lerp(segments[s].startTaper, segments[s].endTaper, section / sections)
                        startRadius = segments[s].startRadius
                        endRadius = segments[s].endRadius
                        linearRadius = utils_.utils.lerp(segments[s].startRadius, segments[s].endRadius, section / (segmentLength / branchRingSpacing))
                        normalizedCurve = (1.0 - branchTipRadius) * tVal + utils_.utils.sampleCurveStem(treeGen, tVal)
                        ...
                        endRadius = segments[s].endRadius
                        linearRadius = utils_.utils.lerp(segments[s].startRadius, segments[s].endRadius, section / (segmentLength / branchRingSpacing))
                        normalizedCurve = (1.0 - branchTipRadius) * tValBranch + utils_.utils.sampleCurveBranch(treeGen, tValBranch, segments[s].clusterIndex) * context.scene.taperFactorList[segments[s].clusterIndex].taperFactor 
                        radius = linearRadius * normalizedCurve
                ...
                for c in range(0, sections): 
                    ...
                    pos = utils_.utils.sampleSplineC(segments[s].start, controlPt1, controlPt2, segments[s].end, c / sections)
                    nextPos = utils_.utils.sampleSplineC(segments[s].start, controlPt1, controlPt2, segments[s].end, (c + 1) / sections)
                    
                    linearRadius = utils_.utils.lerp(segments[s].startRadius, segments[s].endRadius, c / (segmentLength / branchRingSpacing))
                    normalizedCurve = (1.0 - branchTipRadius) * tVal + utils_.utils.sampleCurveStem(treeGen, tVal)
                    ...
                    nextLinearRadius = utils_.utils.lerp(segments[s].startRadius, segments[s].endRadius, (c + 1) / (segmentLength / branchRingSpacing))
                    nextNormalizedCurve = (1.0 - branchTipRadius) * nextTval + utils_.utils.sampleCurveStem(treeGen, nextTval)
        ...
        for leafClusterIndex in range(0, len(leafClusterSettingsList)):
            ...
            if len(startNodesNextIndexStartTvalEndTval) > 0:
                ...
                for leafIndex in range(0, int(nrLeaves)):
                    ...
                    startPointRadius = utils_.utils.lerp(data.startNode.radius, data.startNode.next[startNodeNextIndex].radius, data.t)
                    
                    verticalAngle = utils_.utils.lerp(leafClusterSettingsList[leafClusterIndex].leafVerticalAngleBranchStart, leafClusterSettingsList[leafClusterIndex].leafVerticalAngleBranchEnd, utils_.utils.lerp(data.startNode.tValBranch, data.startNode.next[data.startNodeNextIndex].tValBranch, data.t))
                 
                    rotateAngle = utils_.utils.lerp(leafClusterSettingsList[leafClusterIndex].leafRotateAngleBranchStart, leafClusterSettingsList[leafClusterIndex].leafRotateAngleBranchEnd, utils_.utils.lerp(data.startNode.tValBranch, data.startNode.next[data.startNodeNextIndex].tValBranch, data.t))
                    
                    tiltAngle = utils_.utils.lerp(leafClusterSettingsList[leafClusterIndex].leafTiltAngleBranchStart, leafClusterSettingsList[leafClusterIndex].leafTiltAngleBranchEnd, utils_.utils.lerp(data.startNode.tValBranch, data.startNode.next[data.startNodeNextIndex].tValBranch, data.t))
