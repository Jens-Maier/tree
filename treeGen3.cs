...
public struct startPointData
{
    ...
    public startPointData(Vector3 StartPoint, Vector3 OutwardDir, node StartNode, int StartNodeIndex, int StartNodeNextIndex, float T, Vector3 Tangent)
    {
...
public class node
{
    ...
    public void getAllStartNodes(List<startNodeInfo> startNodesNextIndexStartTvalEndTval, List<List<startNodeInfo>> branchNodesNextIndexStartTvalEndTval, int activeBranchIndex, List<int> nrSplitsPassedAtStartNode, int nrSplitsPassed, float startHeightGlobal, float startHeightCluster, float endHeightGlobal, float endHeightCluster, int level, List<bool> parentClusterBools, int parentLevelCounter, bool calledFromAddLeaves)
    {
        if (calledFromAddLeaves == true)
        {
            ...
            Debug.Log("bools in getAllStartNodes: " + s);
        }
        ...
        if (clusterIndex == -1)
        {
            ...
            foreach (node n in next)
            {
                n.getAllStartNodes(startNodesNextIndexStartTvalEndTval, branchNodesNextIndexStartTvalEndTval, activeBranchIndex, nrSplitsPassedAtStartNode, nrSplitsPassed, startHeightGlobal, startHeightCluster, endHeightGlobal, endHeightCluster, level + 1, parentClusterBools, parentLevelCounter, calledFromAddLeaves);
            }
        }
        else
        {
            ...
            if (calledFromAddLeaves == true)
            {
                Debug.Log("in getAllStartNodes: calledFromAddLeaves == true: cluster: " + clusterIndex + ", " + str);
            }

            //int tempClusterIndex = clusterIndex + 1;
            //if (calledFromAddLeaves == true) // 
            //{
            //    tempClusterIndex = clusterIndex;
            //}

            // Determine which cluster index to check in parentClusterBools.
            // For branches: parentClusterBools[0] is the stem, so we check clusterIndex + 1 (branches grow from previous cluster).
            // For leaves: leafParentClusterBools[0] is branch cluster 0, so we check clusterIndex (leaves attach directly to the cluster).
            int parentClusterIndex;
            if (calledFromAddLeaves)
            {
                // When adding leaves, use the current cluster index.
                parentClusterIndex = clusterIndex;
            }
            else
            {
                // When adding branches, use the next cluster index (offset by 1).
                parentClusterIndex = clusterIndex + 1;
            }

            if (parentClusterBools.Count > parentClusterIndex)
            {
                if (calledFromAddLeaves == true)
                {
                    Debug.Log("parentClusterBools.Count: " + parentClusterBools.Count + ", clusterIndex: " + clusterIndex);
                }
                if (parentClusterBools[parentClusterIndex] == true)
                {
                    if (calledFromAddLeaves == true)
                    {
                        gen.debugListGreen.Add(point);
                        Debug.Log("cluster: " + clusterIndex + ", tempClusterIndex: " + parentClusterIndex + ", tValBranch: " + tValBranch + ", startHeightCluster: " + startHeightCluster);
                    }
                    if (next.Count > 0)
                    {
                        ...
                        for (int n = 0; n < next.Count; n++)
                        {
                            ...
                            if (tValGlobal >= startHeightGlobal && tValGlobal <= endHeightGlobal)
                            {
                                ...
                                if (tB > startHeightCluster && tA < endHeightCluster)
                                {
                                    ...
                                    if (calledFromAddLeaves == true)
                                    {
                                        Debug.Log("clusterIndex: " + clusterIndex + ", tempClusterIndex: " + parentClusterIndex + ": adding node to startNodesNextIndexStartTvalEndTval");
                                    }
                                    ...
                                    if (activeBranchIndex != -1)
                                    {
                                        ...
                                        // Debug.Log("adding point to debugListGreen! " + point);
                                        if (calledFromAddLeaves == true)
                                        {
                                            // gen.debugListGreen.Add(point);
                                        }
                                    ...
                                    else
                                    {
                                        if (calledFromAddLeaves == true)
                                        {
                                            Debug.Log("activeBranchIndex = -1: clusterIndex: " + clusterIndex + ", tempClusterIndex: " + parentClusterIndex + ", startNode: " + point);
                                        }
                                    }
                                }
                ...
                else
                {
                    foreach (node n in next)
                    {
                        n.getAllStartNodes(startNodesNextIndexStartTvalEndTval, branchNodesNextIndexStartTvalEndTval, activeBranchIndex, nrSplitsPassedAtStartNode, nrSplitsPassed, startHeightGlobal, startHeightCluster, endHeightGlobal, endHeightCluster, level, parentClusterBools, parentLevelCounter, calledFromAddLeaves);
                    }
                    ...
                    foreach (List<node> c in branches) // branchNodesNextIndex: one list<(node, int)> for each branch
                    {
                        ...
                        foreach (node n in c)
                        {
                            ...
                            n.getAllStartNodes(startNodesNextIndexStartTvalEndTval, branchNodesNextIndexStartTvalEndTval, activeBranchIndex, nrSplitsPassedAtStartNode, nrSplitsPassed, startHeightGlobal, startHeightCluster, endHeightGlobal, endHeightCluster, level, parentClusterBools, parentLevelCounter + 1, calledFromAddLeaves);
                        }
                        //branchIndex += 1; // test!
                    }
                }
...
public class treeGen3 : MonoBehaviour
{
    ...
    public void addBranches()
    {
        ...
        for (int clusterIndex = 0; clusterIndex < nrBranchClusters; clusterIndex++)
        {
                ...
                if (parentClusterBools[clusterIndex][j] == true)
                {
                    if (j == 0)
                    {
                        ...
                        rootNode.getAllStartNodes(startNodesNextIndexStartTvalEndTval, branchNodesNextIndexStartTvalEndTval, -1, nrSplitsPassedAtStartNode, 0,
                                                    ...
                                                    branchesEndHeightCluster[clusterIndex], j, parentClusterBools[clusterIndex], 0, false);
                    ...
                    else
                    {
                        ...
                        foreach (node n in parentNodes)
                        {
                            n.getAllStartNodes(startNodesNextIndexStartTvalEndTval, branchNodesNextIndexStartTvalEndTval, activeBranchIndex, nrSplitsPassedAtStartNode, 0,
                                                branchesStartHeightGlobal[clusterIndex], branchesStartHeightCluster[clusterIndex], branchesEndHeightGlobal[clusterIndex],
                                                branchesEndHeightCluster[clusterIndex], j, parentClusterBools[clusterIndex], 0, false); // startNodesNextIndex.Count = 49 OK !!!
            ...
            if (startNodesNextIndexStartTvalEndTval.Count > 0)
            {
                ...
                //Debug.Log("parameters for calculateSegmentLengthsAndTotalLength() in Branches(): startNodesNextIndexStartTvalEndTval.Count: " + startNodesNextIndexStartTvalEndTval.Count +
                //        ", branchNodesNextIndexStartTvalEndTval.Count: " + branchNodesNextIndexStartTvalEndTval.Count + ", clusterIndex: " + clusterIndex);
                
                float totalLength = calculateSegmentLengthsAndTotalLength(startNodesNextIndexStartTvalEndTval, branchNodesNextIndexStartTvalEndTval, segmentLengths, clusterIndex, useTvalBranch, false);
                ...
                for (int branchIndex = 0; branchIndex < nrBranches[clusterIndex]; branchIndex++) // nrBranches[0] // TODO: get the branch of the startNode!
                {
                    ...
                    startPointData data = generateStartPointData(startNodesNextIndexStartTvalEndTval, segmentLengths, branchPos, false);
                    Vector3 startPoint = data.startPoint;
                    Vector3 outwardDir = data.outwardDir;
                    node startNode = data.startNode;
                    int startNodeIndex = data.startNodeIndex;
                    int startNodeNextIndex = data.startNodeNextIndex;
                    float t = data.t;
                    Vector3 tangent = data.tangent;


                    Vector3 startPointTangent = sampleSplineTangentT(startNode.point,
                                                                     startNode.next[startNodeNextIndex].point,
                                                                     tangent,
                                                                     startNode.next[startNodeNextIndex].tangent[0],
                                                                     t);
                    ...
                    float globalVerticalAngle = fLerp(verticalAngleCrownStart[clusterIndex], verticalAngleCrownEnd[clusterIndex], startNode.tValGlobal);
                    float branchVerticalAngle = 0f;
                    if (verticalAngleBranchEnd.Count > clusterIndex)
                    {
                        branchVerticalAngle = fLerp(verticalAngleBranchStart[clusterIndex], verticalAngleBranchEnd[clusterIndex], startNode.tValBranch);
                    }
                    else
                    {
                    ...
                    node branch = new node(startPoint, branchDir, branchCotangent, startNode.tValGlobal, tValBranchStart, taper * taperFactor[clusterIndex], this, null, clusterIndex, clusterRingResolution[clusterIndex]); // taper[] for each level
                    ...
                    float lengthToTip = startNode.lengthToTip();
                    lengthToTip -= t * vLength(startNode.next[startNodeNextIndex].point - startNode.point);
                    ...
                    branch.next.Add(new node(startPoint + branchDir * branchLength, branchDir, branchCotangent, startNode.tValGlobal, tValBranchEnd, taper * taperFactor[clusterIndex], this, branch, clusterIndex, clusterRingResolution[clusterIndex]));
                    //Debug.Log("branches count: " + startNodesNextIndex[startNodeIndex].Item1.branches.Count + ", n: " + startNodeNextIndex);
                    if (startNode.branches.Count < startNodeNextIndex + 1)
                    {
                        for (int m = 0; m < startNode.next.Count; m++)
                        {
                            startNode.branches.Add(new List<node>());
                        }
                    }

                    startNode.branches[startNodeNextIndex].Add(branch);
    ...
    public startPointData generateStartPointData(List<startNodeInfo> startNodesNextIndexStartTvalEndTval, List<float> segmentLengths, float branchPos, bool calledFromAddLeaves)
    {
        ...
        for (int i = 0; i < segmentLengths.Count; i++)
        {
            if (accumLength + segmentLengths[i] >= branchPos)
            {
                ...
                if (calledFromAddLeaves == true)
                {
                    // Ensure t is above startTval if startTval > 0                             21               !!! ERROR HERE !!!                     21                                             28
                    Debug.Log("startNodesNextIndexStartTvalEndTval.Count: " + startNodesNextIndexStartTvalEndTval.Count + ", startNodeIndex: " + startNodeIndex + ", segmentLengths.Count: " + segmentLengths.Count);
                    // for false | true | true: 
                    //                                                                          21                                                      0...20                                          21
                }

                float startTval = startNodesNextIndexStartTvalEndTval[startNodeIndex].startTval; // todo // ERROR: segmentLengths.Count > startNodesNextIndexStartTvalEndTval.Count
        ...
        if (calledFromAddLeaves == true)
        {
            Debug.Log("startNodesNextIndexStartTvalEndTval.Count: " + startNodesNextIndexStartTvalEndTval.Count + ", segmentLengths.Count: " + segmentLengths.Count + ", startNodeIndex: " + startNodeIndex);
        }
        ...
        return new startPointData(startPoint, outwardDir, nStart, startNodeIndex, startNodeNextIndex, t, tangent);
    }

    public float calculateSegmentLengthsAndTotalLength(List<startNodeInfo> startNodesNextIndexStartTvalEndTval, List<List<startNodeInfo>> branchNodesNextIndexStartTvalEndTval, List<float> segmentLengths, int clusterIndex, bool useTvalBranch, bool calledFromAddLeaves)
    {
        ...
        if (useTvalBranch == true) // ERROR HERE -> do not use startNodes -> use branches! (branches have startNodes inbetween!!!)
        {
            for (int branchNode = 0; branchNode < branchNodesNextIndexStartTvalEndTval.Count; branchNode++)
            {
                for (int i = 0; i < branchNodesNextIndexStartTvalEndTval[branchNode].Count; i++)
                {
                    ...
                    //Debug.Log("cluster: " + clusterIndex + ", adding length: " + segLenAbove);
        ...
        else // OK
        {
            for (int i = 0; i < startNodesNextIndexStartTvalEndTval.Count; i++)
            {
                ...
                float frac;
                if (tB_global - tA_global == 0f) // ERROR HERE??? always 0 for branches? -> useTvalBranch for leaves on???
                {
                    //Debug.Log("cluster: " + clusterIndex + ", ERROR: tB - tA == 0f, tA: " + tA_global + ", tB: " + tB_global + ", startNode.tValGlobal: " + startNodeNextIndexStartTvalEndTval.startNode.tValGlobal + ", nextIdx: " + startNodeNextIndexStartTvalEndTval.nextIndex);
                    frac = 0f;
                }
                else
                {
                ...
                //           ", segmentTEnd[" + debugEndIndex + "] added: " + segmentTEnd[segmentTEnd.Count - 1]);
                segmentLengths.Add(segLenAbove);
                //Debug.Log("cluster: " + clusterIndex + ", adding length: " + segLenAbove); // ERROR HERE !!!
                totalLength += segLenAbove;
            }
        }
        ...
        //if (calledFromAddLeaves == true)
        //{
        //    Debug.Log("end of calculateSegmentLengthsAndTotalLength (called from addLeaves): clusterIndex: " + clusterIndex + ", totalLength: " + totalLength + ", segmentLengths.Count: " + segmentLengths.Count);
        //}
        //else
        //{
        //    Debug.Log("end of calculateSegmentLengthsAndTotalLength: clusterIndex: " + clusterIndex + ", totalLength: " + totalLength + ", segmentLengths.Count: " + segmentLengths.Count);    
        //}
    ...
    public void addLeaves()
    {
        ...
        for (int clusterIndex = 0; clusterIndex < nrBranchClusters; clusterIndex++)
        {
            if (leafParentClusterBools[clusterIndex] == true)
            {
                ...
                Debug.Log("parameters for calculateSegmentLengthsAndTotalLength() in addLeaves(): clusterStartNodesNextIndexStartTvalEndTval.Count: " + clusterStartNodesNextIndexStartTvalEndTval.Count +
                            ", clusterBranchNodesNextIndexStartTvalEndTval.Count: " + clusterBranchNodesNextIndexStartTvalEndTval.Count + ", i: " + clusterIndex);

                totalLength += calculateSegmentLengthsAndTotalLength(clusterStartNodesNextIndexStartTvalEndTval, clusterBranchNodesNextIndexStartTvalEndTval, clusterSegmentLengths, clusterIndex, true, true);
                // TODO: useTvalBranch ...
                // TEST: useTvalBranch = TRUE !!! -> leafPos !!!
                ...
                Debug.Log("addLeaves: cluster " + clusterIndex + ": clusterStartNodesNextIndexStartTvalEndTval.Count: " + clusterStartNodesNextIndexStartTvalEndTval.Count + // cluster 0: 160, cluster 1: 0
                                                      ", clusterBranchNodesNextIndexStartTvalEndTval.Count: " + clusterBranchNodesNextIndexStartTvalEndTval.Count + // cluster 0: 40, cluster 1: 56 (== nrBranches(cluster0))
                                                      ", totalLength: " + totalLength + // cluster 0: 120.0201, cluster 1: 120.0201 // ERROR -> cluster1: same as cluster 0 ???
                                                      ", segmentLengths.Count: " + segmentLengths.Count); // cluster 0: 160, cluster1: 160 // ERROR -> cluster1: same as cluster 0 ???
            }
        }
        Debug.Log("addLeaves: leafStartNodesNextIndexStartTvalEndTval.Count: " + leafStartNodesNextIndexStartTvalEndTval.Count); // leafParentCluster 0: 160, leafParentCluster 1: 0


        if (leafStartNodesNextIndexStartTvalEndTval.Count > 0) // clusterBranchNodes ???
        {
            for (int leafIndex = 0; leafIndex < nrLeaves; leafIndex++)
            {
                // NEW: startNodeIndex calculation (AI) ---------------------------
                ...
                //-----------
                if (segmentLengths.Count != leafStartNodesNextIndexStartTvalEndTval.Count)
                {
                    Debug.LogWarning("segmentLengths.Count (" + segmentLengths.Count + ") != leafStartNodesNextIndexStartTvalEndTval.Count (" + leafStartNodesNextIndexStartTvalEndTval.Count + ")");
                }

                startPointData leafStartPointData = generateStartPointData(leafStartNodesNextIndexStartTvalEndTval, segmentLengths, leafPos, true);

                debugPointsRed.Add(leafStartPointData.startPoint);
        ...
        {
            Debug.Log("ERROR: leafStartNodesNextIndexStartTvalEndTval.Count == 0");
        }
    }
...
// 
        ...
        if (allSegments != null)
        {
            ...
            {
                Gizmos.DrawSphere(v, gizmoRadius * 0.7f);
            }
            Gizmos.color = Color.green;
            ...
            {
                Gizmos.DrawSphere(v, gizmoRadius * 0.2f);
            }
